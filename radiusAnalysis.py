...
def trueFillRate(data,missingDictionary,missingList):
    '''
    Scrubs missing values from the data set. Returns scrubbed data set
    and a report on the fill rate of the updated data set.
    Args:
        data : A data frame 
    ...
    Returns:
        A 2 element list
        Position 0: df2: A data frame where white spaces have been stripped and missing values replaced with NaN
        Position 1: fillRate(df2): A data frame with the fill rates for each column in df2
    '''
...
cleanedDF['phone'] = cleanPhones(cleanedDF['phone'] ) #remove non-numeric characters
